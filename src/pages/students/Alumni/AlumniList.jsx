...
import AlumniAddWork from "./AlumniAddWork";
...
const AlumniList = () => {
  ...
  // Helper function to sort alumni alphabetically by name (case-insensitive)
  const sortAlumnisAlphabetically = (alumnis) => {
    const sorted = [...alumnis].sort((a, b) => {
      ...
      const nameA = String(a.applicantNameEng || "")
        .toLowerCase()
        .trim();
      const nameB = String(b.applicantNameEng || "")
        .toLowerCase()
        .trim();
  ...
  const getAllAlumnis = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await getAllAlumniStudents();
  ...
  useEffect(() => {
    ...
    if (searchTerm.trim() !== "") {
      filtered = allAlumnis.filter((alumni) => {
        ...
        return (
          (alumni.applicantNameEng?.toLowerCase() || "").includes(
            searchLower
          ) ||
          (alumni.email?.toLowerCase() || "").includes(searchLower) ||
          (alumni.contactNo?.toLowerCase() || "").includes(searchLower) ||
          (alumni.programName?.toLowerCase() || "").includes(searchLower) ||
          (alumni.studentRegNo?.toLowerCase() || "").includes(searchLower)
        );
      });
    }
  ...
  const handleOpenWorkDialog = (alumni) => {
                      ...
                      <TableCell>
                        <Box display="flex" gap={1}>
                          <Tooltip title="Add Work Experience">
                            <IconButton
                              color="primary"
                              onClick={() => handleOpenWorkDialog(alumni)}
                              size="small"
                              sx={{
                                "&:hover": {
