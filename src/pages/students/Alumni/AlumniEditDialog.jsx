...
// Helper function to get today's date in BS format
const getTodayBS = () => {
...
// Helper function to get today's date in AD format
const getTodayAD = () => {
...
const AlumniEditDialog = ({ open, onClose, alumniData, onUpdate }) => {
  ...
  const authConfig = getAuthConfigSafe(); // Renamed to avoid conflict
    ...
    const fetchProgramData = async () => {
      try {
        const response = await axios.get(
          ...
          authConfig // Use renamed variable
        );
  ...
  useEffect(() => {
    if (open && alumniData) {
      ...
      // Set form values from alumniData
      Object.keys(alumniData).forEach((key) => {
        if (alumniData[key] !== null && alumniData[key] !== undefined) {
          // Handle date fields specifically
          if (key === "doBEng" || key === "issueDateEng") {
            setValue(key, formatDateFromBackend(alumniData[key]));
          } else {
            setValue(key, alumniData[key]);
          }
        }
      });

      // Set specific fields that might have different names
      setValue("fiscalYearId", alumniData.fiscalYearID);
      ...
      setValue("program", alumniData.programMgmtId);
      setValue("ethnicity", alumniData.ethinicity); // Note: backend uses "ethinicity" (misspelled)

      // Set gender value
      setValue("gender", alumniData.gender);

      // Set date of birth values
      setValue("dobNepali", alumniData.doBNepali);
      setValue("dobEng", formatDateFromBackend(alumniData.doBEng));

      // Set level if available
      if (alumniData.levelName) {
        setLevel(alumniData.levelName);
      }
  ...
  const onSubmit = async (data) => {
    try {
      ...
      formData.append("id", alumniData.id); // Include the alumni ID for update
      ...
      formData.append("doBNepali", data.dobNepali);
      ...
      await axios.put(`${backendUrl}/Graduation/${alumniData.id}`, formData, {
        headers: {
      ...
      toast.success("Alumni data updated successfully", { autoClose: 1500 });
      onUpdate();
      onClose();
    } catch (error) {
      toast.error("An error occurred while updating the alumni data.");
      console.error(error);
    } finally {
      setIsLoading(false);
    }
...
export default AlumniEditDialog;
