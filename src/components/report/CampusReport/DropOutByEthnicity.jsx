...
import { getUniDropoutData } from "../../../services/services";

const DropOutByEthnicity = ({ roleName }) => {
    ...
    const handleYearChange = (event) => {
        ...
        useEffect(() => {
            const fetchAllCampuses = async () => {
                try {
                    const response = await getCampusForSelection()
    ...
    useEffect(() => {
        const fetchData = async () => {
            const response = await getUniDropoutData({ fiscalId: selectedFiscalYearId, campusId })
            setApiData(response)
        }
        fetchData()
    ...
    const rows = apiData?.map((item, index) => ({
        ...
        total: item.total,
        chhetri: item?.ethinicGroup?.Chhetri,
        brahman: item?.ethinicGroup?.Brahman,
        madhesi: item?.ethinicGroup?.Madhesi,
        dalit: item?.ethinicGroup?.Dalit,
        muslim: item?.ethinicGroup?.Muslim,
        tharu: item?.ethinicGroup?.Tharu,
        janajati: item?.ethinicGroup?.Janajati,
        others: item?.ethinicGroup?.Others,
    }))
    ...
    const selectedCampusObject = allCampuses.find(campus => campus.id === campusId) || null;
                ...
                <Table style={{ borderCollapse: "collapse", border: "1px solid #ddd" }}>
                    ...
                    {filteredRows.length > 0 ?
                        (
                            ...
                            <TableRow>
                                <TableCell colSpan={11} sx={{ textAlign: "center", padding: "8px" }}>
                                    No students
                                </TableCell>
                            </TableRow>
                        )}
...
export default DropOutByEthnicity;
