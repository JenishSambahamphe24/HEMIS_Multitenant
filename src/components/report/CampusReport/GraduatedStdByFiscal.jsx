...
const GraduatedStdByFiscal = ({ fiscalId }) => {
    ...
    const fetchData = async () => {
        try {
            const response = await getGraduationOfLast5FY();
    ...
    // Safely extract fiscal years
    const fiscalYears = [
        ...new Set(
            (apiData || []).flatMap(item =>
                (item?.fiscalData || []).map(fiscal => fiscal?.year).filter(Boolean)
            )
        ),
    ].sort((a, b) => {
        const [aYear] = (a || "").split("/");
            ...
            <TableContainer sx={{ border: "1px solid #ddd" }}>
                <Table style={{ borderCollapse: "collapse", border: "1px solid #ddd" }}>
                    <TableHead style={{ backgroundColor: "#2A629A" }}>
                        <TableRow>
                            <TableCell sx={{ border: "1px solid #ddd", color: "#fff", padding: "4px", textAlign: "center" }} rowSpan={2}>
                                S.No.
                            </TableCell>
                            <TableCell sx={{ border: "1px solid #ddd", color: "#fff", padding: "4px", textAlign: "center" }} rowSpan={2}>
                                Program
                            </TableCell>
                            <TableCell sx={{ border: "1px solid #ddd", color: "#fff", padding: "4px", textAlign: "center" }} colSpan={fiscalYears.length}>
                                Last 5 fiscal years
                            </TableCell>
                        </TableRow>
                        <TableRow>
                            {fiscalYears.map((year, index) => (
                                <TableCell key={index} sx={{ border: "1px solid #ddd", color: "#fff", padding: "4px", textAlign: "center" }}>
                                    {year}
                                </TableCell>
                            ))}
                        </TableRow>
                    ...
                    <TableBody>
                        {filteredRows.length > 0 ? (
                            ...
                            <TableRow>
                                <TableCell colSpan={fiscalYears.length + 2} sx={{ textAlign: "center", padding: "10px" }}>
                                    No Data Available
                                </TableCell>
                            </TableRow>
                        )}
...
export default GraduatedStdByFiscal;
