...
import { AddressContext } from "./AddressInfo";
import { GeneralInfoContext } from "./GeneralInfo";
import { GuardianInfoContext } from "./GuardianInfo";
import { RegistrationInfoContext } from "./RegistrationInfo";
...
import { getBatchById, getFacultyByFacultyId, getLevelById, getProgramById } from "../../../../services/services";
import toast from "react-hot-toast";
import { useNavigate } from "react-router-dom";
import {config} from '@config';

const FinalTransferReview = ({ handleBack }) => {
  ...
  const { studentInfo, getLogoURL } = useContext(GeneralInfoContext);
  const { studentAddress } = useContext(AddressContext);
  const { guardianInfo } = useContext(GuardianInfoContext);
  ...
  const handleRegister = async () => {
    ...
    formData.append("year", registrationInfo.year);
    formData.append("semester", registrationInfo.semester);
    ...
    formData.append("programType", registrationInfo.programType);
    ...
    // Transfer 
    formData.append("isTransferredIn", registrationInfo.isTransferredIn);
    formData.append("transferredFrom", registrationInfo.transferredFrom);
    formData.append("transferStatus", true);
    formData.append("transferredDate", registrationInfo.transferredDate);
    formData.append("transferDoc", registrationInfo.transferDoc);
    ...
    try {
      ...
      const response = await axios.post(
        `${backendUrl}/Student/EnrolledOfTransferred`,
        formData,
        config
      );
      ...
      navigate('/student-management/student-transfer-in')
    } catch (err) {
  ...
  const registrationInfoStudent = registrationInfo
    ? [
      ...
      { name: "Transferred from:", detail: registrationInfo.transferredFrom },
      { name: "Transferred Date:", detail: registrationInfo.transferredDate },
    ]
    : [];
...
export default FinalTransferReview;
